...
def get_values_from_batch( batch):
    inputs = []
    y_task = []
    y_domain = []
    for domain in batch.keys():
        inputs.append(batch[domain]['video'])
        y_domain.append(batch[domain]['domain'])
        #print(batch[domain]['label'].shape)
        y_task.append(batch[domain]['label'])


    inputs = torch.cat(inputs, dim=0)
    y_task = torch.cat(y_task, dim=0)
    y_domain = torch.cat(y_domain, dim=0)

    return inputs, y_task, y_domain
