...
from loading.load_frames import *
...
class Source_Datasets(data_utl.Dataset):

    def __init__(self, split, args, transforms=None, flownet=None, args_flow_net=None):

        # print("prepare dataset cme")
        self.is_inference = args.is_inference
        self.data = make_dataset(args.split_file, split, args.root,
                                 args.original_mode, args.num_classes, args.nb_frames, args.is_inference,
    ...
    def get_video(self, index, domain):
        ...
        imgs, list_imgs = load_frames(self.root, vid, start_f, self.nb_frames,
                                      self.edge_type, self.data_aug,
    ...
    def __getitem__(self, index):
        ...
        """
        idx_1 = random.randint(0, index)
        vid_1, label_1, domain_1 = self.get_video(idx_1, 1.0)

        idx_2 = random.randint(0, index)
        vid_2, label_2, domain_2 = self.get_video(idx_2, 2.0)

        idx_3 = random.randint(0, index)
        vid_3, label_3, domain_3 = self.get_video(idx_3, 3.0)

        dic_values = {}

        # idx_4 = random.randint(0, index)
        # vid_4, label_4, domain_4 = self.get_video(idx_4, 4.0)
        if self.args.ablation != 'model2':
            dic_values['domain_0'] = {}
            dic_values['domain_0']['video'] = vid_1
            dic_values['domain_0']['label'] = label_1
            dic_values['domain_0']['domain'] = domain_1

            dic_values['domain_1'] = {}
            dic_values['domain_1']['video'] = vid_2
            dic_values['domain_1']['label'] = label_2
            dic_values['domain_1']['domain'] = domain_2

            dic_values['domain_2'] = {}
            dic_values['domain_2']['video'] = vid_3
            dic_values['domain_2']['label'] = label_3
            dic_values['domain_2']['domain'] = domain_3
        else:
            dic_values['domain_0'] = {}
            dic_values['domain_0']['video'] = vid_1
            dic_values['domain_0']['label'] = label_1
...
class Target_Datasets(data_utl.Dataset):

    def __init__(self, split, args, transforms=None, flownet=None, args_flow_net=None):
        print("prepare target dataset  {}  {}".format(args.target_split_file, args.target_root))
        self.is_inference = True
        self.data = make_dataset(args.target_split_file, split, args.target_root,
                                 args.original_mode, args.num_classes, args.nb_frames, self.is_inference,
                                 args.label_smoothing)
        self.args = args
        self.nb_frames = args.nb_frames
        self.blur_kernel = args.blur_kernel
        self.operator_kernel = args.operator_kernel
        self.split_file = args.target_split_file
        self.transforms = transforms
        self.flownet = flownet
        self.args_flownet = args_flow_net
        self.original_mode = args.original_mode
    ...
    def get_video(self, index, domain):
        ...
        return frames_pt, torch.from_numpy(label).squeeze(), torch.tensor(domain).long().squeeze()
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: (image, target) where target is class_index of the target class.
        """
        vid, label, domain = self.get_video(index, 4.0)

        dic_values = {}

        dic_values['target'] = {}
        dic_values['target']['video'] = vid
        dic_values['target']['label'] = label
        dic_values['target']['domain'] = domain
...
class Source_Dataset(data_utl.Dataset):

    def __init__(self, split, args, split_file, root,  transforms=None, flownet=None, args_flow_net=None, domain=0):
        self.is_inference = True
        self.split_file = split_file
        self.root = root
        print("prepare  dataset  {}  {}".format(self.split_file, self.root))
        self.data = make_dataset(self.split_file, split, self.root,
                                 args.original_mode, args.num_classes, args.nb_frames, self.is_inference,
                                 args.label_smoothing)
        self.domain = domain
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: (image, target) where target is class_index of the target class.
        """
        vid, label, domain = self.get_video(index, self.domain)
